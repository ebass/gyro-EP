...
! Crossover energy calculated in Alpha (deprecated) or manually.

  call MPI_BCAST(EXPRO_energy_c,size(EXPRO_energy_c),MPI_DOUBLE_PRECISION,0,comm,ierr)

  ! Now all processes have these profiles, so no further 
  ! broadcasts are required.

  if (i_proc==0) print *, '[EXPRO_pread complete]'
  call EXPRO_compute_derived
  if (i_proc==0) print *, '[EXPRO_compute_derived complete]'
