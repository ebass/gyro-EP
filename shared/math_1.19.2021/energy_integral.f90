...
subroutine energy_integral(n_energy,energy_max,n_kinetic,n_x,ir_norm,energy,w_energy,velocity_dist_method_vec,energy_c)
  ...
  real, intent(in) :: energy_c(n_kinetic,n_x)
  ! 
...
!!!fixed    energy_max(is) = 1.
    a = 0.
    do i = 1, n_energy
      ec32 = (energy_c(is,ir_norm))**1.5
      b = ( (ec32+a**1.5)*((ec32+1.)/ec32)**(1./n_energy) - ec32)**(2./3.)
      energy(i,is) = 0.5*(b+a)
!      print *, energy_c(ir_norm), ec32, a, b
      do ix = 1, n_x
        ec32 = (energy_c(is,ix))**1.5
        w_energy(i,is,ix) = log((ec32+b**1.5)/(ec32+a**1.5)) / &
                              log((ec32+1.)/ec32)
!        if (ISNAN(w_energy(i,is,ix)) .or. (w_energy(i,is,ix)==0.0) ) print *, 'in energy_integral', i, ix, w_energy(i,is,ix), a, b, energy_c(is,ix)
      enddo ! Radius loop, ix
      a = b
    enddo ! Energy loop, i
